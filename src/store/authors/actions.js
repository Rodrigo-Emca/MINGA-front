...
const read_author = createAsyncThunk("read_author", async ({ author_id }) => {
  try {
    let res = await axios.get("http://localhost:8000/api/authors/" + author_id);
...
const read_all_authors = createAsyncThunk("read_all_authors", async () => {
  try {
    ...
    let res = await axios.get("http://localhost:8000/api/authors/admin/prueba", headers);
    //console.log(res)
...
const update_active_author = createAsyncThunk(
  ...
  async ({ _id, active }) => {
    ...
    let headers = { headers: { 'Authorization': `Bearer ${token}` } }
    try {
      let response = await axios.put(`http://localhost:8000/api/authors/admin/${_id}`, { active: active }, headers);
      return {
        author: response.data.author,
        success: true
      }
